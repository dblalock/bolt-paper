...
Multiplying matrices is among the most fundamental and most computationally demanding tasks in machine learning and scientific computing. Consequently, the task of efficiently approximating matrix products has received a great deal of attention in recent years. Existing approximation methods work by reducing the number of multiply-add operations, reducing the bitwidth of the data, or by otherwise reducing the cost of each operation. We describe a method that, given time to preprocess one of the two matrices, eliminates the multiply-add operations entirely. Moreover, experiments across a wide range of datasets show that our method obtains a significantly better efficiency vs. quality tradeoff than existing approaches. Our method also has theoretical guarantees, based on an approach to bounding sums of dependent variables that may be of independent interest.
