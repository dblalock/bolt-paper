...
% Beyond these existing guarantees,

Our central theoretical result is a generalization guarantee for the overall approximation error of \ours, stated below. See Appendix~\ref{sec:maddnessMath} for a proof and additional analysis, including a discussion of algorithmic complexity. Besides this main guarantee, we also inherit all of the guarantees for \textsc{Bolt}\text{} \cite{bolt}, modulo a small amount of additional error from averaging integers rather than summing exactly. This follows from Bolt's guarantees depending only on the quantization errors, rather than the method used to obtain them.
